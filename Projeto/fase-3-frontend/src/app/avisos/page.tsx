      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <motion.div {...fadeIn} transition={{ delay: 0.05 }}>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Comunicações e avisos da plataforma Softinsa Badges
          </p>
        </motion.div>
        ...
        {loading ? (
          ...
          <Card className="border border-border shadow-sm">
            <CardContent className="p-8 text-center text-muted-foreground">
              ...
              <p className="text-sm">Não há avisos ativos de momento.</p>
            </CardContent>
          </Card>
        ) : (
                    ...
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground shrink-0">
                        <span className="flex items-center gap-1">
                          ...
                          {new Date(notice.starts_at).toLocaleDateString("pt-PT")}
                          {notice.ends_at && ` → ${new Date(notice.ends_at).toLocaleDateString("pt-PT")}`}
                        </span>
                      </div>
                    </div>
                    {notice.target_roles && notice.target_roles.length > 0 && (
                      <div className="flex items-center gap-1.5 mt-1">
                        ...
                        <span className="text-xs text-muted-foreground">
                          Para: {notice.target_roles.map((r) => (ROLE_LABELS as any)[r] ?? r).join(", ")}
                        </span>
                      </div>
                    )}
                  </CardHeader>
