...
export default function ChangePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(t("page.changePw.error"));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4 w-full">
      <div className="max-w-md w-full bg-card rounded-2xl shadow-card border border-border p-8">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-foreground">{t("page.changePw.title")}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            ...
            {loading ? t("page.changePw.saving") : t("page.changePw.submit")}
          </button>
        </form>
      </div>
    </div>
