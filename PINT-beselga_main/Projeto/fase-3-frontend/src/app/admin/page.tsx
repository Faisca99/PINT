      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.1 }}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: t("admin.dash.stat.users"),       value: usersKpi?.total ?? "—",      icon: Users,        color: "bg-primary/10 text-primary",          link: "/admin/utilizadores" },
              { label: t("admin.dash.stat.activeBadges"),       value: badgeCount,                  icon: Award,        color: "bg-green-500/10 text-green-600",       link: "/admin/badges" },
              { label: t("admin.dash.stat.totalApps"),  value: summary?.total ?? "—",       icon: Send,         color: "bg-blue-500/10 text-blue-600",         link: "/relatorios" },
              { label: t("admin.dash.stat.pendingReview"),    value: (summary?.pending_tm ?? 0) + (summary?.pending_sll ?? 0), icon: Clock, color: "bg-amber-500/10 text-amber-600", link: "/relatorios" },
            ].map((s, i) => (
            ...
            <Card className="border border-border shadow-card">
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <Users className="h-4 w-4 text-accent" /> {t("admin.dash.usersByRole")}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    { label: t("admin.dash.role.consultants"),       value: usersKpi.consultants,          color: "text-blue-600",   bg: "bg-blue-50" },
                    { label: t("admin.dash.role.tms"),   value: usersKpi.talent_managers,      color: "text-amber-600",  bg: "bg-amber-50" },
                    { label: t("admin.dash.role.slls"), value: usersKpi.service_line_leaders, color: "text-purple-600", bg: "bg-purple-50" },
                    { label: t("admin.dash.role.admins"),   value: usersKpi.admins,               color: "text-red-600",    bg: "bg-red-50" },
                  ].map((u) => (
            ...
            <Card className="border border-border shadow-card h-full">
              ...
              <CardContent>
                {loading ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground text-sm">{t("admin.dash.noMonthly")}</div>
                ) : (
                  <div className="space-y-2.5">
                    ...
                    <div className="flex items-center gap-4 mt-3 pt-3 border-t border-border">
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <div className="w-3 h-3 rounded-full bg-primary" />{t("admin.dash.legend.approved")}
                      </div>
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <div className="w-3 h-3 rounded-full bg-primary/15" />{t("admin.dash.legend.total")}
                      </div>
                    </div>
                  </div>
                )}
            ...
            <Card className="border border-border shadow-card h-full">
              ...
              <CardContent>
                {loading ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground text-sm">{t("admin.dash.noData")}</div>
                ) : (
            ...
            <Card className="border border-border shadow-card h-full">
              ...
              <CardContent className="space-y-3">
                {loading ? (
                  ...
                  { label: t("admin.dash.pendingTM"),    value: summary.pending_tm,  icon: Clock,         color: "bg-amber-500/10 text-amber-600",  bar: "bg-amber-400" },
                  { label: t("admin.dash.pendingSLL"),   value: summary.pending_sll, icon: AlertTriangle, color: "bg-blue-500/10 text-blue-600",    bar: "bg-blue-400" },
                  { label: t("admin.dash.approved"),        value: summary.approved,    icon: CheckCircle2,  color: "bg-green-500/10 text-green-600",  bar: "bg-green-400" },
                  { label: t("admin.dash.rejected"),       value: summary.rejected,    icon: XCircle,       color: "bg-red-500/10 text-red-600",      bar: "bg-red-400" },
                ].map((s) => {
            ...
            <Card className="border border-border shadow-card h-full">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base flex items-center gap-2">
                    ...
                    {t("admin.dash.recentApps")}
                  </CardTitle>
                  <Link href="/relatorios">
                    <Button variant="ghost" size="sm" className="text-accent text-xs">
                      {t("admin.dash.viewAll")} <ChevronRight className="ml-1 h-3 w-3" />
                    </Button>
                  </Link>
                </div>
              </CardHeader>
              <CardContent className="p-0">
                {loading ? (
                  ...
                  <div className="p-6 text-center text-muted-foreground text-sm">{t("admin.dash.noApps")}</div>
                ) : (
                  <div className="divide-y divide-border">
                    {recent.map((app) => {
                      const isClosed = app.status === "closed";
                      const statusLabel = isClosed
                        ? (app.final_result === "approved" ? t("admin.dash.approved") : t("admin.dash.rejected"))
                        : app.status === "submitted" ? t("admin.dash.submitted") : t("admin.dash.inValidation");
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.45 }}>
          <Card className="border border-border shadow-card">
            <CardHeader className="pb-3">
              <CardTitle className="text-base">{t("admin.dash.quickAccess")}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                {[
                  { label: t("admin.dash.stat.users"),   icon: Users,       url: "/admin/utilizadores", color: "text-primary" },
                  { label: t("admin.dash.quick.badges"),          icon: Award,       url: "/admin/badges",       color: "text-green-600" },
                  { label: t("admin.dash.quick.structure"),    icon: Layers,      url: "/admin/estrutura",    color: "text-blue-600" },
                  { label: t("admin.dash.quick.notices"),          icon: Bell,        url: "/admin/avisos",       color: "text-amber-600" },
                  { label: t("admin.dash.quick.reports"),      icon: BarChart3,   url: "/relatorios",         color: "text-purple-600" },
                  { label: t("admin.dash.quick.rgpd"),            icon: ShieldCheck, url: "/admin/rgpd",         color: "text-red-600" },
                ].map((item) => (
