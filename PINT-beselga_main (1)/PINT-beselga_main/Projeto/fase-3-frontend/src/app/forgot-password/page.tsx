...
export default function ForgotPasswordPage() {
  ...
  const handleRequestReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(t("page.forgot.errRequest"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(t("page.forgot.errToken"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const stepTitles = {
    email: { heading: t("page.forgot.step1.heading"), sub: t("page.forgot.step1.sub") },
    reset: { heading: t("page.forgot.step2.heading"), sub: t("page.forgot.step2.sub") },
    done: { heading: t("page.forgot.step3.heading"), sub: t("page.forgot.step3.sub") },
  };
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          >
            <h1 className="text-5xl font-bold text-primary-foreground leading-tight mb-6">
              {t("page.forgot.hero1")}
              <br />
              <span className="text-accent">{t("page.forgot.hero2")}</span>
            </h1>
            <p className="text-lg text-primary-foreground/70 leading-relaxed">
              {t("page.forgot.heroSub")}
            </p>
          </motion.div>
        ...
        >
            ...
            {step === "email" && (
              <motion.form key="email"
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>{t("page.forgot.sendRequest")} <ChevronRight className="h-4 w-4" /></>}
                </button>
                ...
                <Link href="/login" className="flex items-center justify-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
                  <ArrowLeft className="h-3.5 w-3.5" /> {t("page.forgot.backToLogin")}
                </Link>
              </motion.form>
            )}
              ...
              <motion.form key="reset"
                ...
                {token && (
                  <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-700">
                    <p className="font-medium mb-1">{t("page.forgot.academicToken")}</p>
                    <p className="font-mono break-all">{token}</p>
                  </div>
                )}

                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground block">{t("page.forgot.recoveryToken")}</label>
                  <input type="text" required value={token} onChange={(e) => setToken(e.target.value)}
                    placeholder={t("page.forgot.pasteToken")}
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground block">{t("auth.newPassword")}</label>
                  <div className="relative">
                ...
                <button type="submit" disabled={loading}
                  className="w-full h-11 flex items-center justify-center gap-2 rounded-md gradient-primary text-primary-foreground font-medium text-sm hover:opacity-90 transition-opacity disabled:opacity-50">
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>{t("page.forgot.resetPassword")} <ChevronRight className="h-4 w-4" /></>}
                </button>
            ...
            {step === "done" && (
              <motion.div key="done"
                ...
                <Link href="/login"
                  ...
                  {t("page.forgot.goToLogin")} <ChevronRight className="h-4 w-4" />
                </Link>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="mt-12 pt-6 border-t border-border">
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>{t("auth.e2eEncryption")}</span>
            </div>
          </div>
        </motion.div>
      </div>
