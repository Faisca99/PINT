...
export default function UtilizadoresPage() {
  ...
  useEffect(() => {
    api.get("/users")
      ...
      .catch(() => setError("Não foi possível carregar os utilizadores."))
  ...
  const exportExcel = () => {
    const data = filtered.map((u) => ({
      ...
      "Perfil": u.role,
      "Área": u.area_name ?? "",
      "Service Line": u.service_line_name ?? "",
      "Badges Obtidos": u.badge_count,
      "Pontos": u.total_points,
      "Último Acesso": u.last_login_at ? new Date(u.last_login_at).toLocaleDateString("pt-PT") : "Nunca",
    }));
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-muted-foreground mt-1 text-sm">
                Todos os consultores ativos na plataforma
              </p>
            </div>
            <Button onClick={exportExcel} disabled={loading || users.length === 0} variant="outline" className="gap-2">
          ...
          <Card className="border border-border shadow-card">
            <CardHeader className="pb-3">
              <CardTitle className="text-base">
                {loading ? "A carregar..." : `${filtered.length} consultor${filtered.length !== 1 ? "es" : ""}`}
              </CardTitle>
            </CardHeader>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border bg-muted/30">
                        <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Consultor</th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden md:table-cell">Área</th>
