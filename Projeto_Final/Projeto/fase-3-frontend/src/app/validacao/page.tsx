...
export default function ValidacaoInboxPage() {
  ...
  const statusFilters = user.role === "talent_manager"
    ? [{ key: "all", label: t("label.all") }, { key: "submitted", label: t("label.submitted") }, { key: "in_validation", label: t("label.inValidation") }, { key: "closed", label: t("label.closed") }]
    : [{ key: "all", label: t("label.all") }, { key: "in_validation", label: t("label.inValidation") }, { key: "closed", label: t("label.closed") }];
      ...
      <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-1 text-sm">
                {user.role === "talent_manager" ? t("page.validacao.sub.tm") : t("page.validacao.sub.sll")}
              </p>
            </div>
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.1 }}>
          <div className="grid grid-cols-3 gap-4">
            {[
              { icon: Clock,         color: "bg-amber-500/10 text-amber-600", value: pendingCount, label: t("label.pendentes") },
              { icon: CheckCircle2,  color: "bg-green-500/10 text-green-600",  value: applications.filter((a) => a.status === "closed" && a.final_result === "approved").length, label: t("label.aprovadas") },
              { icon: XCircle,       color: "bg-red-500/10 text-red-600",      value: applications.filter((a) => a.status === "closed" && a.final_result === "rejected").length, label: t("label.rejeitadas") },
            ].map((s) => (
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.15 }}>
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                type="text"
                placeholder={t("label.searchPersonBadge")}
                value={search}
            ...
            <CardContent className="p-0">
              {loading ? (
                ...
                <div className="p-8 text-center text-muted-foreground">
                  ...
                  <p className="text-sm">{t("label.noAppsFilter2")}</p>
                </div>
              ) : (
                <>
                          ...
                          <div className="flex items-center gap-4 ml-4">
                            ...
                            <Link href={`/validacao/${app.id}`}>
                              <Button size="sm" variant="outline" className="gap-1 h-8 text-xs">
                                {t("label.review")} <ChevronRight className="h-3 w-3" />
                              </Button>
                            </Link>
                          </div>
                        </div>
