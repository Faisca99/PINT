...
import { Users, Award, Clock, CheckCircle2, ChevronRight, RefreshCw, BarChart3, CalendarClock } from "lucide-react";
...
interface ExpiringRow { id: number; days_left: number; consultant_name: string; badge_name: string; level_code: string | null; expires_at: string; }
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          <motion.div {...fadeIn} transition={{ delay: 0.05 }}>
            <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
              ...
              {user ? `${getGreeting(user, user.lastLoginAt)}, ${user.name.split(" ")[0]}! 👋` : t("dashboard.tm.title")}
            </h1>
            <p className="text-muted-foreground mt-1 text-sm">{t("dashboard.tm.sub")}</p>
          </motion.div>
          ...
          <motion.div {...fadeIn} transition={{ delay: 0.1 }}>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: t("dashboard.tm.consultants"), value: users.filter((u) => u.role === "consultant").length, icon: Users, color: "bg-blue-500/10 text-blue-600" },
                { label: t("dashboard.tm.pendingTM"), value: summary?.pending_tm ?? 0, icon: Clock, color: "bg-amber-500/10 text-amber-600" },
                { label: t("label.aprovadas"), value: summary?.approved ?? 0, icon: CheckCircle2, color: "bg-green-500/10 text-green-600" },
                { label: t("dashboard.tm.totalApps"), value: summary?.total ?? 0, icon: Award, color: "bg-primary/10 text-primary" },
              ].map((s, i) => (
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-base flex items-center gap-2">
                      ...
                      {t("dashboard.tm.pendingReview")} ({pending.length})
                    </CardTitle>
                    <Link href="/validacao">
                ...
                <CardContent>
                  {pending.length === 0 ? (
                    <div className="text-center py-6 text-muted-foreground">
                      ...
                      <p className="text-sm">{t("dashboard.tm.noPending")}</p>
                    </div>
                  ) : (
                ...
                <CardContent>
                  {topConsultants.length === 0 ? (
                    <div className="text-center py-6 text-muted-foreground">
                      ...
                      <p className="text-sm">{t("dashboard.tm.noConsultants")}</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
          ...
          {/* Badges próximos da expiração (Req. TM 20) */}
          <motion.div {...fadeIn} transition={{ delay: 0.35 }}>
            <Card className="border border-border shadow-card">
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <CalendarClock className="h-4 w-4 text-orange-500" />
                  {t("dashboard.expiring.title")} ({expiring.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
                {expiring.length === 0 ? (
                  <div className="text-center py-6 text-muted-foreground">
                    <CheckCircle2 className="h-8 w-8 mx-auto mb-2 opacity-30" />
                    <p className="text-sm">{t("dashboard.expiring.none")}</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {expiring.slice(0, 10).map((e) => {
                      const d = Number(e.days_left);
                      const isExpired = d < 0;
                      return (
                        <div key={e.id} className="flex items-center justify-between p-3 rounded-lg border border-border">
                          <div className="min-w-0">
                            <div className="text-sm font-medium text-foreground truncate">{e.consultant_name}</div>
                            <div className="text-xs text-muted-foreground truncate">
                              {e.level_code ? `${e.level_code} · ` : ""}{e.badge_name}
                            </div>
                          </div>
                          <span className={`text-xs font-medium ml-3 shrink-0 px-2 py-0.5 rounded-full ${isExpired ? "bg-red-100 text-red-700" : "bg-orange-100 text-orange-700"}`}>
                            {isExpired
                              ? t("dashboard.expiring.expired")
                              : d === 0
                                ? t("page.myBadges.expiresToday")
                                : t("page.myBadges.expiresInDays").replace("{n}", String(d))}
                          </span>
                        </div>
                      );
                    })}
