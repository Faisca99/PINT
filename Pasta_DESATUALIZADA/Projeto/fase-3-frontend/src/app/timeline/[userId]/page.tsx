...
const MILESTONES = [
  { count: 1,  label: "🏆 Primeiro Badge!" },
  { count: 3,  label: "⚡ 3 Badges — Trio de Excelência!" },
  { count: 5,  label: "💎 5 Badges — Colecionador!" },
  { count: 10, label: "👑 10 Badges — Mestre!" },
];

const EVENT_CONFIG = {
  badge_earned: { dot: "bg-yellow-500", label: "Badge obtido",             color: "text-yellow-600", bg: "bg-yellow-50 border-yellow-200" },
  achievement:  { dot: "bg-purple-500", label: "Conquista desbloqueada",   color: "text-purple-600", bg: "bg-purple-50 border-purple-200" },
};

const APP_STATUS: Record<string, { dot: string; label: string; color: string; bg: string }> = {
  submitted:     { dot: "bg-blue-500",  label: "Candidatura submetida", color: "text-blue-600",  bg: "bg-blue-50 border-blue-200" },
  in_validation: { dot: "bg-amber-500", label: "Em validação (SLL)",    color: "text-amber-600", bg: "bg-amber-50 border-amber-200" },
  approved:      { dot: "bg-green-500", label: "Candidatura aprovada",  color: "text-green-600", bg: "bg-green-50 border-green-200" },
  rejected:      { dot: "bg-red-500",   label: "Candidatura rejeitada", color: "text-red-600",   bg: "bg-red-50 border-red-200" },
  open:          { dot: "bg-slate-400", label: "Candidatura criada",    color: "text-slate-600", bg: "bg-slate-50 border-slate-200" },
};
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <motion.div {...fadeIn} transition={{ delay: 0.0 }}>
          <Link href="/utilizadores" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground gap-1.5 mb-2">
            <ArrowLeft className="h-4 w-4" /> Consultores
          </Link>
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-muted-foreground">
            ...
            <p className="text-sm">Sem eventos na timeline deste consultor.</p>
          </div>
        ) : (
                      ...
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <div>
                          ...
                          <div className="text-sm font-medium text-foreground mt-0.5">
                            {event.badge_name}
                            {event.level_code && <span className="text-xs text-muted-foreground ml-2">(Nível {event.level_code})</span>}
                          </div>
                        </div>
                        <span className="text-xs text-muted-foreground shrink-0">
                          {new Date(event.occurred_at).toLocaleString("pt-PT", { day: "2-digit", month: "short", year: "numeric" })}
                        </span>
                      </div>
                    </div>
                  </motion.div>
