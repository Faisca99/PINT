...
export default function LembretesPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.title || !form.scheduled_for) { setFormError("Título e data são obrigatórios."); return; }
    setSaving(true);
    setFormError(null);
    try {
      ...
      setForm(EMPTY);
      fetchReminders();
    } catch { setFormError("Erro ao criar lembrete."); }
  ...
  const handleDismiss = async (id: number) => {
    ...
    } catch { alert("Erro ao dispensar lembrete."); }
  };
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className={`text-xs shrink-0 ${isOverdue ? "text-red-600 font-medium" : "text-muted-foreground"}`}>
              {isOverdue ? "⚠️ " : ""}{new Date(r.scheduled_for).toLocaleDateString("pt-PT")}
            </span>
          </div>
          ...
          {!r.dismissed_at && (
            <button onClick={() => handleDismiss(r.id)}
              ...
              <CheckCircle2 className="h-3 w-3" /> Marcar como concluído
            </button>
          )}
        </div>
      </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
                <Bell className="h-6 w-6 text-accent" /> Lembretes
              </h1>
              <p className="text-muted-foreground mt-1 text-sm">
                Define lembretes para cumprir requisitos de badges a tempo
              </p>
            </div>
            <Button onClick={() => { setShowForm(true); setForm(EMPTY); setFormError(null); }} className="gap-2">
              <PlusCircle className="h-4 w-4" /> Novo Lembrete
            </Button>
          </div>
        </motion.div>
              ...
              <CardContent>
                <form onSubmit={handleCreate} className="space-y-3">
                  <input type="text" value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))}
                    placeholder="Ex: Submeter certificado OutSystems até..."
          ...
          <>
            ...
            {upcoming.length > 0 && (
              <motion.div {...fadeIn} transition={{ delay: 0.15 }}>
                <h2 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5">
                  <Clock className="h-4 w-4 text-amber-500" /> Próximos ({upcoming.length})
                </h2>
            ...
            {active.length === 0 && dismissed.length === 0 && (
              <div className="p-8 text-center text-muted-foreground">
                ...
                <p className="text-sm">Sem lembretes. Cria um para não te esqueceres de cumprir os requisitos!</p>
              </div>
            )}
            ...
            {dismissed.length > 0 && (
              <motion.div {...fadeIn} transition={{ delay: 0.2 }}>
                <h2 className="text-sm font-semibold text-muted-foreground mb-2 flex items-center gap-1.5">
                  <CheckCircle2 className="h-4 w-4" /> Concluídos ({dismissed.length})
                </h2>
