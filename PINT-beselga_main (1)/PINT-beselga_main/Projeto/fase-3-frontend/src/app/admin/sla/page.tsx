...
export default function AdminSlaPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      fetchSlas();
    } catch { setError(t("admin.sla.errCreate")); }
  ...
  const handleToggle = async (id: number, active: boolean) => {
    ...
    } catch { alert(t("admin.sla.errToggle")); }
  };
      ...
      <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
                <Timer className="h-6 w-6 text-accent" /> {t("admin.sla.title")}
              </h1>
            ...
            <Button onClick={() => { setShowForm(true); setError(null); }} className="gap-2">
              <PlusCircle className="h-4 w-4" /> {t("admin.sla.new")}
            </Button>
          </div>
        </motion.div>
            ...
            <Card className="border border-primary/30 shadow-card">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base">{t("admin.sla.new")}</CardTitle>
              ...
              <CardContent>
                <form onSubmit={handleCreate} className="space-y-4">
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t("admin.sla.team")}</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t("admin.sla.limitHours")}</label>
                      <input type="number" min="1" value={form.limit_hours}
                        onChange={(e) => setForm((p) => ({ ...p, limit_hours: e.target.value }))}
                        className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t("admin.sla.warnPercent")}</label>
                  ...
                  <div className="flex gap-3">
                    <Button type="submit" disabled={saving} className="flex-1">
                      ...
                      {saving ? t("admin.struct.creating") : t("admin.sla.create")}
                    </Button>
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.1 }}>
          <Card className="border border-border shadow-card">
            <CardHeader className="pb-3">
              <CardTitle className="text-base">{loading ? t("admin.common.loading") : `${slas.length} ${slas.length !== 1 ? t("admin.sla.countPlural") : t("admin.sla.countSingular")}`}</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {loading ? (
                ...
                <div className="p-8 text-center text-muted-foreground">
                  ...
                  <p className="text-sm">{t("admin.sla.empty")}</p>
                </div>
              ) : (
                  ...
                  {slas.map((s) => (
                    <div key={s.id} className="flex items-center justify-between px-5 py-4">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${s.is_active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                            {s.is_active ? t("admin.badges.active") : t("admin.badges.inactive")}
                          </span>
                        </div>
                        <p className="text-xs text-muted-foreground">
                          {t("admin.sla.limit")}: <strong>{s.limit_hours}h</strong> · {t("admin.sla.alertAt")} {s.warning_at_percent}% ({Math.round(s.limit_hours * s.warning_at_percent / 100)}h)
                        </p>
                      </div>
                      <Button size="sm" variant="ghost" onClick={() => handleToggle(s.id, !s.is_active)}
                        ...
                        {s.is_active ? t("admin.struct.deactivate") : t("admin.struct.activate")}
                      </Button>
                    </div>
                  ))}
                </div>
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.15 }}>
          <div className="flex items-start gap-2 p-4 rounded-xl border border-amber-200 bg-amber-50/50 text-xs text-amber-700">
            ...
            <div>
              <p className="font-medium mb-1">{t("admin.sla.howTitle")}</p>
              <p>{t("admin.sla.howText")}</p>
            </div>
          </div>
        </motion.div>
      </div>
