...
export default function Dashboard() {
  ...
  const levelProgress = Object.entries(LEVEL_COLORS).map(([code, color]) => {
    const earned = myBadges.filter((b) => b.level_code?.startsWith(code)).length;
    return { code, color, label: t(`level.${code}`), earned };
  });
      ...
      {!user ? null : (
        <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="flex items-start gap-3 p-4 rounded-xl border border-amber-300 bg-amber-50 text-amber-800">
                ...
                <div>
                  <p className="text-sm font-semibold">
                    {expiringBadges.length} {t("dashboard.expiring")}
                  </p>
                  <p className="text-xs mt-0.5">
                    {expiringBadges.map((b) => `${b.badge_name} (${formatDate(b.expires_at)})`).join(" · ")}
                  </p>
                </div>
              </div>
            </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: t("dashboard.totalPoints"), value: loading ? "—" : points, icon: Zap, accent: true },
              { label: t("dashboard.badgesObtained"), value: loading ? "—" : approvedCount, icon: Award },
              { label: t("dashboard.inValidation"), value: loading ? "—" : pendingCount, icon: Clock },
              { label: t("dashboard.area"), value: user.area ?? "—", icon: Target, small: true },
            ].map((stat, i) => (
              ...
              <Card className="border border-border shadow-card h-full">
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg flex items-center gap-2">
                    ...
                    {t("dashboard.progress")} — {user.area ?? t("dashboard.allLevels")}
                  </CardTitle>
                </CardHeader>
                    ...
                    <div key={code} className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {earned > 0 ? (
                          ...
                          <span className="text-xs text-muted-foreground">{t("dashboard.noBadges")}</span>
                        )}
                      </div>
                    </div>
                  ))}
                ...
                <CardContent className="space-y-3">
                  {loading ? (
                    ...
                    <div className="text-center py-6 text-muted-foreground">
                      ...
                      <p className="text-sm">{t("dashboard.noBadgesYet")}</p>
                      <Link href="/badges">
                        <Button size="sm" variant="outline" className="mt-2">{t("dashboard.viewCatalog")}</Button>
                      </Link>
                    </div>
              ...
              <Card className="border border-border shadow-card">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Link href="/badges">
                      <Button variant="ghost" size="sm" className="text-accent text-sm">
                        {t("dashboard.viewCatalogLink")} <ChevronRight className="ml-1 h-4 w-4" />
                      </Button>
                    </Link>
                  </div>
                </CardHeader>
              ...
              <CardContent>
                {loading ? (
                  ...
                  <div className="text-center py-6 text-muted-foreground">
                    ...
                    <Link href="/badges">
                      <Button size="sm" variant="outline" className="mt-3">{t("dashboard.viewBadges")}</Button>
                    </Link>
                  </div>
                ) : (
