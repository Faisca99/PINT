...
export default function DashboardSLPage() {
  ...
  // Comparação por área (Bónus SLL 1 — mesma área/experiência)
  ...
  // Áreas dos consultores (para a comparação por área)
  const consultantAreas = useMemo(() =>
    Array.from(new Set(consultants.map((c) => c.area_name).filter(Boolean))),
    [consultants]
  );
  // Consultores a comparar (filtrados pela área selecionada = mesma área/experiência)
  const compared = useMemo(() =>
    compareArea === "all" ? consultants : consultants.filter((c) => c.area_name === compareArea),
    [consultants, compareArea]
  );
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    ...
                    {consultantAreas.length > 1 && (
                      ...
                      >
                        <option value="all">Todas as áreas</option>
                        {consultantAreas.map((a) => <option key={a} value={a!}>{a}</option>)}
                      </select>
                    )}
                  </div>
                  {compareArea !== "all" && (
                    <p className="text-xs text-muted-foreground mt-1">
                      A comparar {compared.length} consultor{compared.length !== 1 ? "es" : ""} da área <span className="font-medium text-foreground">{compareArea}</span>
                    </p>
                  )}
                </CardHeader>
                <CardContent>
                  ...
                  <div className="space-y-2 mb-5">
                    {[...compared]
                      ...
                      .slice(0, 8)
                      .map((c) => {
                        const max = Math.max(...compared.map((x) => Number(x.badge_count)), 1);
                  ...
                  <div className="space-y-2">
                    {[...compared]
                      ...
                      .slice(0, 8)
                      .map((c) => {
                        const max = Math.max(...compared.map((x) => Number(x.total_points)), 1);
