...
export default function AdminUtilizadoresPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (!form.full_name || !form.email || !form.password) {
      setFormError("Nome, email e password são obrigatórios.");
      return;
    }
    setSaving(true);
    setFormError(null);
    try {
      ...
      setFormError("Erro ao criar utilizador. O email pode já estar registado.");
    } finally {
      setSaving(false);
    }
  };
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.05 }}>
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-muted-foreground mt-1 text-sm">Criar, editar e gerir perfis</p>
            </div>
          ...
          <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-card border border-border rounded-2xl shadow-xl w-full max-w-md p-6">
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-lg font-bold text-foreground">Novo Utilizador</h2>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                {[
                  { label: "Nome completo", key: "full_name", type: "text", placeholder: "Nome Sobrenome" },
                  { label: "Email", key: "email", type: "email", placeholder: "nome@softinsa.pt" },
                  { label: "Password inicial", key: "password", type: "password", placeholder: "••••••••" },
                ].map((f) => (
                  <div key={f.key}>
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Área preferencial (opcional)</label>
                  <select
                    value={form.area_id}
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.15 }}>
          <Card className="border border-border shadow-card">
            <CardHeader className="pb-3">
              <CardTitle className="text-base">{loading ? "A carregar..." : `${filtered.length} utilizador${filtered.length !== 1 ? "es" : ""}`}</CardTitle>
            </CardHeader>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border bg-muted/30">
                        {["Utilizador", "Perfil", "Área", "Estado", "Último acesso", "Ações"].map((h) => (
                        ...
                        <motion.tr key={user.id} {...fadeIn} transition={{ delay: 0.03 * i }} className="hover:bg-muted/20 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${user.account_status === "active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                              {user.account_status === "active" ? "Ativo" : "Suspenso"}
                            </span>
                          </td>
