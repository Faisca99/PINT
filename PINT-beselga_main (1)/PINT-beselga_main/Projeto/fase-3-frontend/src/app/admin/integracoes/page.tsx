...
export default function AdminIntegracoesPage() {
  ...
  const handleSave = async (provider: string) => {
    ...
    } catch { alert(t("admin.integ.errSave")); }
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <motion.div {...fadeIn} transition={{ delay: 0.05 }}>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <Plug className="h-6 w-6 text-accent" /> {t("admin.integ.title")}
          </h1>
          ...
          <motion.div key={p.key} {...fadeIn} transition={{ delay: 0.1 + i * 0.05 }}>
            <Card className="border border-border shadow-card">
              ...
              <CardContent className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">{t("admin.integ.webhookUrl")}</label>
                  <input
                    type="url"
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-foreground">{t("admin.integ.activate")}</span>
                  </label>
                  <Button size="sm" onClick={() => handleSave(p.key)} disabled={saving === p.key} className="gap-2">
                    {saving === p.key ? (
                      <RefreshCw className="h-3.5 w-3.5 animate-spin" />
                    ) : saved === p.key ? (
                      <><CheckCircle2 className="h-3.5 w-3.5 text-green-400" /> {t("admin.integ.saved")}</>
                    ) : (
                      <><Save className="h-3.5 w-3.5" /> {t("admin.common.save")}</>
                    )}
                  </Button>
                </div>
              </CardContent>
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.25 }}>
          <div className="p-4 rounded-xl border border-border bg-muted/20 text-xs text-muted-foreground space-y-1">
            <p className="font-medium text-foreground">{t("admin.integ.howTitle")}</p>
            <p>• <strong>Teams:</strong> {t("admin.integ.howTeams")}</p>
            <p>• <strong>Slack:</strong> {t("admin.integ.howSlack")}</p>
            <p>{t("admin.integ.howNote")}</p>
          </div>
        </motion.div>
      </div>
    </AppLayout>
