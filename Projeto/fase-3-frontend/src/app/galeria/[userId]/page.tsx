  ...
  if (notFound || !gallery) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4 p-8">
        ...
        <h1 className="text-2xl font-bold text-foreground">Galeria não encontrada</h1>
        <p className="text-muted-foreground text-center">Este consultor não existe ou não tem badges públicos.</p>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="bg-card border border-border rounded-2xl shadow-xl p-8 mb-8 text-center">
          ...
          <div className="flex items-center justify-center gap-8 mt-6">
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground mt-0.5">Badges Verificados</div>
            </div>
            <div className="h-10 w-px bg-border" />
            <div className="text-center">
              <div className="text-3xl font-bold text-yellow-600">{totalPoints}</div>
              <div className="text-xs text-muted-foreground mt-0.5">Pontos Totais</div>
            </div>
          </div>

          <div className="flex items-center justify-center gap-2 mt-4 text-xs text-green-600">
            <ShieldCheck className="h-4 w-4" />
            Badges verificados pela Softinsa
          </div>
        </div>
        ...
        {gallery.badges.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p>Este consultor ainda não tem badges públicos.</p>
          </div>
        ) : (
                ...
                <div key={badge.id} className={`bg-card border rounded-2xl shadow-card p-5 flex flex-col ${expired ? "opacity-60" : "border-border"}`}>
                  ...
                  <div className="mt-3 pt-3 border-t border-border space-y-1.5">
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      ...
                      {expired ? (
                        <span className="text-red-600">⚠️ Expirou em {new Date(badge.expires_at!).toLocaleDateString("pt-PT")}</span>
                      ) : (
                        `Obtido em ${new Date(badge.awarded_at).toLocaleDateString("pt-PT")}`
                      )}
                    </div>
